...
STRIX_PATH = ROOT_PATH / "Tugas_Akhir"
...
def save_uploaded_file(file, type):
    if type == "inp":
        with open(os.path.join("Tugas_Akhir/app/input_files/Input_File.xlsx"), "wb") as f:
            f.write(file.getbuffer())
    elif type == "pipe":
        with open(os.path.join("Tugas_Akhir/app/pipeline_files/Pipeline_File.xlsx"), "wb") as f:
            f.write(file.getbuffer())
    ...
    if PIPELINE_FILE or st.session_state.PIPELINE_FILE:
        ...
        if tiein:
            ...
            if (lat1 != "") and (lon1 != ""):
                ...
                lat1 = max(-90, min(90, float(lat1)))
                # Ensure longitude is in the range -180 to 180
                lon1 = max(-180, min(180, float(lon1)))
                point1 = (lat1, lon1)
            ...
            if (lat2 != "") and (lon2 != ""):
                # Ensure latitude is in the range -90 to 90
                lat2 = max(-90, min(90, float(lat2)))
                
                # Ensure longitude is in the range -180 to 180
                lon2 = max(-180, min(180, float(lon2)))
    ...
    if st.session_state.INPUT_FILE:
        ...
        with tab3:
            merged_data = {"Name": [],
                    ...
                    "Lat": [],
                    "Lon": []}
            merged_data["Name"].extend(source_df["UNIQUE NAME"].tolist())
            merged_data["Name"].extend(sink_df["UNIQUE NAME"].tolist())
            merged_data["Type"].extend(["source"] * len(source_df))
            merged_data["Type"].extend(["sink"] * len(sink_df))
            merged_data["Lat"].extend(source_df["Lat"].tolist())
            merged_data["Lat"].extend(sink_df["Lat"].tolist())
            merged_data["Lon"].extend(source_df["Lon"].tolist())
            merged_data["Lon"].extend(sink_df["Lon"].tolist())

            # Add a new column 'Size' based on latitude values
            merged_df = pd.DataFrame(merged_data)
            merged_df['Size'] = merged_df['Lat'].abs()  # Adjust as needed
            # merged['Size'] = merged['Lat'].abs()  # Adjust as needed
            fig5 = px.scatter_mapbox(merged_df, lat="Lat", lon="Lon", hover_name="Name", color="Type", zoom=8, height=1000, width=1200,
                                    size="Size",  # Modify this line
