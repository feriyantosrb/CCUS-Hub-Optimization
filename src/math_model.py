...
MPS_FILE_PATH = os.path.join("Tugas_Akhir/app/solver_files/CO2_network_optimization.mps")
LP_FILE_PATH = os.path.join("Tugas_Akhir/app/solver_files/CO2_network_optimization.lp")
SOL_FILE_PATH = os.path.join("Tugas_Akhir/app/solver_files/CO2_network_optimization.sol")
ILP_FILE_PATH = os.path.join("Tugas_Akhir/app/solver_files/CO2_network_optimization.ilp")
...
class Math_model:
    def __init__(self, nodes, nodesValue, arcs, arcsInfo, paths, nodesCost, duration, target_cap, crf=0.1) -> None:
        ...
        # Inisialisasi soln_arcs_a di sini
        self.soln_arcs_a = {}
        self.soln_sources = {}
        self.soln_sinks = {}
        self.soln_cap_costs = {}
        self.soln_storage_costs = {}
        self.soln_transport_costs_a = {}
    ...
    def _arc_upper_lower_bound_cons(self) -> None: #arc capacity bounds
    ...
    def _single_direction_arc_flow_cons(self) -> None: #single direction arc flow
    ...
    def _node_balance_cons(self) -> None: #flow balance
    ...
    def _demand_balance_cons(self) -> None: #demand balance
    ...
    def _supply_balance_cons(self) -> None: #supply balance
    ...
    def _capture_limit_cons(self) -> None: #capture capacity bounds
    ...
    def _storage_limit_cons(self) -> None: #storage capacity bounds
    ...
    def _capture_target_cons(self) -> None: #target capture
    ...
    def create_constraints(self) -> None:
        self._arc_upper_lower_bound_cons() #arc capacity bounds
        msg = ("'Arc Bounds' constraint: Time elapsed: %.2f seconds"
               % (time.time() - START_TIME))
        print(msg)
        LOGGER.info(msg)
        self._single_direction_arc_flow_cons() #single direction arc flow
        msg = ("'Single Direction' constraint: Time elapsed: %.2f seconds"
               % (time.time() - START_TIME))
        print(msg)
        LOGGER.info(msg)
        self._node_balance_cons() #flow balance
        msg = ("'Supply Balance' constraint: Time elapsed: %.2f seconds"
               % (time.time() - START_TIME))
        print(msg)
        LOGGER.info(msg)
        self._demand_balance_cons() #demand balance
        msg = ("'Demand Balance' constraint: Time elapsed: %.2f seconds"
               % (time.time() - START_TIME))
        print(msg)
        LOGGER.info(msg)
        self._supply_balance_cons() #supply balance
        msg = ("'Supply Balance' constraint: Time elapsed: %.2f seconds"
               % (time.time() - START_TIME))
        print(msg)
        LOGGER.info(msg)
        self._capture_limit_cons() #capture capacity bounds
        msg = ("'Capture Limit' constraint: Time elapsed: %.2f seconds"
               % (time.time() - START_TIME))
        print(msg)
        LOGGER.info(msg)
        self._storage_limit_cons() #storage capacity bounds
        msg = ("'Storage Limit' constraint: Time elapsed: %.2f seconds"
               % (time.time() - START_TIME))
        print(msg)
        LOGGER.info(msg)
        self._capture_target_cons() #target capture
    ...
    def solve_model(self) -> None:
        ...
        if (self.model.NumVars <= 2000) and (self.model.NumConstrs <= 2000):
            ...
            self.use_pulp = True
            cplex_executable_path = 'C:/Program Files/IBM/ILOG/CPLEX_Studio2211/cplex/bin/x64_win64/cplex.exe'
            self.pulp_var, self.pulp_model = LpProblem.fromMPS(MPS_FILE_PATH)
            self.pulp_solver = pl.CPLEX_CMD(path=cplex_executable_path, options=['mipdisplay=0'])
